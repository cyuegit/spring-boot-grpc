...
/**
 * @ClassName: CustomWeightRandomSubchannelStateListener
 * @Description:
 * @Author: chenglong.yue
 * @Date: 2022/3/20 21:05
 */
@Slf4j
class CustomWeightRandomSubchannelStateListener implements LoadBalancer.SubchannelStateListener {
    private final LoadBalancer.Subchannel subchannel;
    private final LoadBalancer.Helper helper;
    private final CustomWeightRandomLoadBalancer loadBalancer;

    public CustomWeightRandomSubchannelStateListener(CustomWeightRandomLoadBalancer customLoadBalancer,
    ...
    public void onSubchannelState(ConnectivityStateInfo stateInfo) {
        CustomWeightRandomRef<ConnectivityState> stateInfoRef = subchannel.getAttributes().get(CustomWeightRandomLoadBalancer.STATE_INFO);
    ...
    private void updateLoadBalancerState() {
        List<LoadBalancer.Subchannel> readySubchannels = loadBalancer.getSubchannelMap()
                .values()
                .stream()
                .filter(s -> s.getAttributes().get(CustomWeightRandomLoadBalancer.STATE_INFO).getValue() == READY)
                .collect(Collectors.toList());

        if (readySubchannels.isEmpty()) {
            log.info("更新 LB 状态为 CONNECTING，没有 READY 的 Subchannel");
            helper.updateBalancingState(CONNECTING, new CustomWeightRandomSubchannelPicker(LoadBalancer.PickResult.withNoResult()));
        } else {
            log.info("更新 LB 状态为 READY，Subchannel 为:{}", readySubchannels.toArray());
            helper.updateBalancingState(READY, new CustomWeightRandomSubchannelPicker(readySubchannels));
        }
    }
}
