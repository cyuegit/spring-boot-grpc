...
/**
 * @ClassName: CustomWeightRandomLoadBalancer
 * @Description: 权重随机
 * @Author: chenglong.yue
 * @Date: 2022/3/20 20:59
 */
@Slf4j
public class CustomWeightRandomLoadBalancer extends LoadBalancer {

    public static final Attributes.Key<CustomWeightRandomRef<ConnectivityState>> STATE_INFO = Attributes.Key.create("state-info");

    private final Helper helper;

    Map<EquivalentAddressGroup, Subchannel> subchannelMap = new ConcurrentHashMap<>();

    public CustomWeightRandomLoadBalancer(Helper helper) {
        this.helper = helper;
    }
    ...
    private CreateSubchannelArgs buildCreateSubchannelArgs(EquivalentAddressGroup e) {
        return CreateSubchannelArgs.newBuilder()
                ...
                .setAttributes(Attributes.newBuilder()
                        .set(STATE_INFO, new CustomWeightRandomRef<>(IDLE))
                        .build())
                .build();
    }
    ...
    private Subchannel processSubchannel(Subchannel subchannel) {
        ...
        subchannel.start(new CustomWeightRandomSubchannelStateListener(this, subchannel, helper));
    ...
    public void handleNameResolutionError(Status error) {
        ...
        helper.updateBalancingState(ConnectivityState.TRANSIENT_FAILURE, new CustomWeightRandomSubchannelPicker(PickResult.withNoResult()));
    }
